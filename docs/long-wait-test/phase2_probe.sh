...
echo "PHASE2_DONE 睡了$((E-S))s boot前=$SB boot後=$EB uptime=$(cut -d' ' -f1 /proc/uptime)s 心跳筆數=$(grep -c '^OK\|^GAP' $D/heartbeat.log) GAP=$(grep -c '^GAP' $D/heartbeat.log) 最後心跳=$(tail -1 $D/heartbeat.log)"
